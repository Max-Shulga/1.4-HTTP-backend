...
export function dataTable() {
  ...
  function createTable(usersData, config) {
    ...
    const users = usersData.fromServer
      ? usersData.usersDataFromServer
      : usersData.usersDataFromLocal;
    ...
    function createTableRow(user, rowIndex, columns, usersData, config) {
      ...
      bodyTr.innerHTML = `<td class="table-body-cell table-cell table-row-${
        rowIndex + 1
      } index-cell">${rowIndex + 1}</td>`;
      ...
      const deleteButton = deleteButtons().getDeleteButton(
        user.id,
        usersData,
        config,
      );
    ...
    function createTableHeader(columns, users) {
      ...
      columns.forEach(({ value: columnName, title: columnValue }, index) => {
        if (users[0].hasOwnProperty(columnName)) {
          ...
          th.className = `table-header-cell table-cell table-column-${
            index + 2
          }`;
